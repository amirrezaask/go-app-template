...
func NewMinio(ctx context.Context, bucketName string, endpoint string, accessID string, secretAccessID string) (*MinioClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("minio client cannot be created: %w", err)
	}
	_, _ = client.HealthCheck(_MINIO_HEALTH_CHECK_AFTER)

	if !client.IsOnline() {
		return nil, fmt.Errorf("minio endpoint is offline")
	}

	exists, err := client.BucketExists(ctx, bucketName)
	if err != nil {
		return nil, fmt.Errorf("minio bucket exists failed: %w", err)
	}

	if !exists {
		err = client.MakeBucket(ctx, bucketName, minio.MakeBucketOptions{})
		if err != nil {
			return nil, fmt.Errorf("cannot make new minio bucket: %w", err)
		}
	}
